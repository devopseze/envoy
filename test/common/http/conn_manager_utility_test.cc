...
TEST_F(ConnectionManagerUtilityTest, UserAgentSetWhenIncomingEmpty) {
  ...
  user_agent_ = "bar";
  TestRequestHeaderMapImpl headers{{"user-agent", ""}, {"x-envoy-downstream-service-cluster", "foo"}};
...
TEST_F(ConnectionManagerUtilityTest, UserAgentSetIncomingUserAgent) {
  ...
  user_agent_ = "bar";
  TestRequestHeaderMapImpl headers{{"user-agent", "foo"}, {"x-envoy-downstream-service-cluster", "foo"}};
...
TEST_F(ConnectionManagerUtilityTest, PreserveExternalRequestId) {
  ...
  TestRequestHeaderMapImpl headers{{"x-request-id", "my-request-id"}, {"x-forwarded-for", "198.51.100.1"}};
  EXPECT_EQ((MutateRequestRet{"134.2.2.11:0", false}),
            callMutateRequestHeaders(headers, Protocol::Http2));
  EXPECT_CALL(random_, uuid()).Times(0);
